...
async function webhook(payload){const url=process.env.DISCORD_WEBHOOK_URL || lerConfig().DISCORD_WEBHOOK_URL; if(!url||url==='COLE_AQUI_SEU_WEBHOOK_DO_DISCORD'){console.log('Webhook não configurado.'); return;} try{await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});}catch(e){console.error('Erro webhook:', e.message);}}
